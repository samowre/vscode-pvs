...
describe("pvs-prover", () => {
    ...
    it(`can execute tactical 'then'`, async () => {
        ...
        const skosimp_response: PvsResponse = await pvsProxy.proofCommand({ proofId: prfid, cmd: "(skosimp*)" });
        //console.dir(skosimp_response, { depth: null });
        expect(skosimp_response.result["prev-cmd"].toLowerCase()).to.deep.equal("(skosimp*)");

        const split_response: PvsResponse = await pvsProxy.proofCommand({ proofId: prfid, cmd: "(split)" });
